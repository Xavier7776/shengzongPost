...
- **全局 UI 层**：`app/layout.tsx` 的 `<Providers>` 内挂 `pointer-events-none fixed` 全局层（CursorGlow z-0、CursorFollower z-40）。`SiteShell` 对 /admin、/dashboard、/onlyus 裸渲染。`SiteShell` 的 `<main>` **不用 `flex-1`**（否则内容不足时 main 被强制拉伸，与 DistortionEffect 组合产生大片空白）。
- **DistortionEffect 高度**：宽高比 `DISTORTION_ASPECT = 6.5`，height = `(min(100vw, 1600px) + 40px) / 6.5`。改此常量时需同步 DistortionEffect.tsx 内 4 处（常量定义 + CSS 变量 + paddingBottom + inner height）+ SiteShell.tsx loading placeholder。

...
// 头像兜底组件：图片加载失败时自动显示首字母色块
function AvatarWithFallback({
  src, name, size, className, bgClass = 'bg-blue-100', textClass = 'text-blue-600 text-sm'
}: {
  src: string | null
  name: string
  size: number
  ...
  return (
    <div className={`rounded-full overflow-hidden flex-shrink-0 ${className ?? ''}`}>
      {src && !err
        ? <Image src={src} alt={name} width={size} height={size} className="w-full h-full object-cover" onError={() => setErr(true)} />
...
function UserCard({ user, currentUserId }: { user: FollowUser; currentUserId?: number }) {
  return (
    <Link href={`/profile/${user.id}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
      <AvatarWithFallback src={user.avatar} name={user.name} size={40} className="w-10 h-10" />
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 mb-6">
          <div className="flex items-start gap-4">
            <AvatarWithFallback src={user.avatar} name={user.name} size={64} className="w-16 h-16" bgClass="bg-blue-600" textClass="text-white text-xl" />
            <div className="flex-1 min-w-0">
